...
<head>
    ...
    <title>Rock the Patch! v3 - Login Log</title>
...
<div class="container">
    ...
    <!-- ### START content-area ### -->
    <div id="content-area">
        <div id="bread-crumbs"><a href="/" title="Home">Home</a> / <a href="/web-admin/main.php" title="Web Admin">Web Admin</a> / Login Log</div>
        <h1>Login Log</h1>
        ...
        <?php
            ...
            if($gLoginStatus == STATUS_LOGGED_IN) {
                if ($isAdmin) {
        ?>
                    <form action="clear-login-log-and-statistics.php" name="clear-login-log-and-statistics" method="post">
                        <p><input type="submit" value="Clear Login Log and Statistics" class="button" /></p>
                    </form>
        <?php
                    $loginLogFilters = array("userId", "success", "loginTime");
                    $loginLogLabels = array("User", "Success", "Time");

                    lib_database::displayFilterForm("Login Log", PAGE_LOGIN_LOG, $loginLogFilters, $loginLogLabels, TABLE_LOGIN_LOG);
                    lib_database::displayFilteredEntries($loginLogFilters, TABLE_LOGIN_LOG, PAGE_LOGIN_LOG);
                } else {
